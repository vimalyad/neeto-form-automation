...
test.describe("Form Creation", () => {
    ...
    test("Create and submit a form", async ({ formCreationPage, page }) => {
        ...
        await test.step("filling invalid email and phoneNumber and error assertion", () => formPage.invalidEmailAndPhoneNumber(
            getInvalidEmailAndPhoneNumber(transformToName(mockUser.firstName, mockUser.lastName)))
        );
