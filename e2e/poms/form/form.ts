...
export default class FormPage {
    ...
    verifyFields = async () => {
        await expect(this.page.getByTestId(FORM_SELECTORS.fullNameGroup).getByTestId(FORM_SELECTORS.formGroupQuestion)).toBeVisible();
        await expect(this.page.getByTestId(FORM_SELECTORS.phoneNumberGroup).getByTestId(FORM_SELECTORS.formGroupQuestion)).toBeVisible();
        await expect(this.page.getByTestId(FORM_SELECTORS.emailGroup).getByTestId(FORM_SELECTORS.formGroupQuestion)).toBeVisible();
    }
    ...
    verifyMultiChoiceQuestionHidden = async () => {
        ...
        await expect(this.page.getByTestId(FORM_SELECTORS.multipleChoiceOptionContainer)).toBeHidden({ timeout: 10000 });
    }

    verifyMultiChoiceQuestionVisible = async () => {
        // verify multi-choice question visible
        await expect(this.page.getByTestId(FORM_SELECTORS.multipleChoiceOptionContainer)).toBeVisible({ timeout: 10000 });
    }
    ...
    gotNameError = async () => {
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidFirstName)).toBeVisible();
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidLastName)).toBeVisible();
    }

    gotPhoneNumberError = async () => {
        const invalidError = this.page.getByText(FORM_ERRORS_TEXT.invalidPhoneNumber);
        const startWithOneError = this.page.getByText(FORM_ERRORS_TEXT.usNumberError);
    ...
    noEmailError = async () => {
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidEmail)).toBeHidden();
    }

    noNameError = async () => {
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidFirstName)).toBeHidden();
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidLastName)).toBeHidden();
    }

    noPhoneNumberError = async () => {
        await expect(this.page.getByText(FORM_ERRORS_TEXT.invalidPhoneNumber)).toBeHidden();
        await expect(this.page.getByText(FORM_ERRORS_TEXT.usNumberError)).toBeHidden();
    }
    ...
    emptyFormSubmitValidation = async () => {
        await this.fillAndSubmitForm({ name: { firstName: '', lastName: '' }, email: '', phoneNumber: '' });
    ...
    private fillPhoneNumber = async (phoneNumber: string) => {
        ...
        await this.page.getByTestId(FORM_SELECTORS.phoneNumberField).fill(phoneNumber);
    }
    ...
    private changeCountryIfNotAlready = async () => {
        const prefix = await this.page.getByTestId(FORM_SELECTORS.phoneNumberPrefix).textContent();
        if (prefix?.trim() === FORM_COUNTRY_DETAILS.unitedStatesPhoneNumberPrefix) return;
        await this.page.locator(FORM_SELECTORS.selectCSS).click();
        await this.page.getByTestId(FORM_SELECTORS.countryCodeSearchInput).fill(FORM_COUNTRY_DETAILS.unitedStates);
        await this.page.getByText(FORM_COUNTRY_DETAILS.unitedStates, { exact: false }).click();
    }
}